...
export async function generateMetadata() {
    const { metaData } = await getMCPSaasPageData();

    return {
        title: metaData?.title || 'MCP for SaaS - viaSocket',
        description: metaData?.description || 'Embed MCP server in your SaaS product - viaSocket',
        keywords: metaData?.keywords || 'MCP, SaaS, integrations, embed',
        openGraph: {
            title: metaData?.title || 'MCP for SaaS - viaSocket',
            description: metaData?.description || 'Embed MCP server in your SaaS product - viaSocket',
...
export default async function MCPSaasPage() {
    const { footerData, faqData, metaData, blogData, navbarData } = await getMCPSaasPageData();
