...
export default function Step4({ total, selectedAddons, basePrice }) {
    const inp =
    ...
    return (
        <div className="animate-[hireStepFade_0.32s_cubic-bezier(0.2,0.7,0.2,1)]">
            <div className="flex items-start gap-2.5 px-3.5 py-3 bg-[rgba(168,32,13,0.04)] border border-[rgba(168,32,13,0.18)] rounded-[10px] mb-3.5 text-[13px] text-[#6b1d10] leading-[1.5]">
            ...
            <div className="border border-[#ececec] rounded-xl p-5 mb-3.5">
                ...
                <div className="mb-3.5">
                    <label className="block text-[13px] font-semibold text-[#111] mb-2">Card number</label>
                    <input className={inp} placeholder="4242 4242 4242 4242" />
                </div>
                <div className="grid grid-cols-2 gap-3.5">
                    <div>
                        <label className="block text-[13px] font-semibold text-[#111] mb-2">Expiry</label>
                        <input className={inp} placeholder="MM / YY" />
                    </div>
                    <div>
                        <label className="block text-[13px] font-semibold text-[#111] mb-2">CVC</label>
            ...
            <div className="bg-[#faf9f4] rounded-xl px-5 py-4.5 py-[18px]">
                ...
                {selectedAddons.map((id) => {
                    ...
                    return (
                        <div key={id} className="flex justify-between items-center py-1 text-[13.5px] text-[#555]">
                            <span>{a.title}</span>
                            <strong className="text-[#111] font-semibold">{a.price}</strong>
                        </div>
                    );
                })}
                <div className="flex justify-between items-center pt-3 mt-2 border-t border-[#ececec] text-[15px] font-bold text-[#111]">
                    <span>Total due today</span>
                    <strong className="text-accent text-xl tracking-[-0.4px]">${total}</strong>
                </div>
            </div>
        </div>
    );
