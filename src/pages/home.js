...
import { getAppCount, getTemplates, getIndustries, getDepartments } from '@/utils/axiosCalls';
...
const Home = ({ metaData, faqData, footerData, securityGridData }) => {
    ...
    const handleSearchVideos = async () => {
        ...
        try {
            // Always try to get all videos first for testing
            const { getVideos } = await import('@/utils/axiosCalls');
            const allVideos = await getVideos(window?.location?.href);
    ...
    const handleSearchBlogs = async () => {
        ...
        try {
            // Create tags from selected apps, industries, and departments
            const appTags = selectedApps.map(app => app.name.toLowerCase());
            const industryTags = selectedIndustries.map(industry => industry.toLowerCase());
            const departmentTags = selectedDepartments.map(dept => dept.toLowerCase());

            // Combine all tags
            const allTags = [...appTags, ...industryTags, ...departmentTags];

            let blogData;

            if (allTags.length > 0) {
                // Use the first two tags for getBlogData function
                const tag1 = allTags[0] || null;
                const tag2 = allTags[1] || null;

                blogData = await getBlogData({ tag1, tag2 }, window?.location?.href);
            } else {
                // Fallback: get all blogs if no tags
                const { getBlogs } = await import('@/utils/axiosCalls');
                blogData = await getBlogs(window?.location?.href);
                blogData = blogData?.slice(0, 6); // Limit to 6 blogs
            }

            setBlogs(blogData || []);
        } catch (error) {
            console.error('Error fetching blogs:', error);
            // Fallback: try to get all blogs
            try {
                const { getBlogs } = await import('@/utils/axiosCalls');
                const fallbackBlogData = await getBlogs(window?.location?.href);
                const limitedBlogs = fallbackBlogData?.slice(0, 6) || [];
