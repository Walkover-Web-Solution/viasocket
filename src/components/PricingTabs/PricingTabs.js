...
export default function PricingTabsClient({ countries }) {
        ...
        const autoDetectLocation = async () => {
            if (countries && countries.length > 0 && !selectedCountry) {
                ...
                try {
                    ...
                    matchingCountry = countries.find((c) => c.country.toLowerCase() === dc.toLowerCase());
                    if (!matchingCountry) {
                        matchingCountry = countries.find((c) => c.country.toLowerCase().includes(dc.toLowerCase()));
                        if (!matchingCountry && dc.length > 4) {
                            matchingCountry = countries.find((c) => dc.toLowerCase().includes(c.country.toLowerCase()));
                        }
                    }
                    if (matchingCountry) {
    ...
    useEffect(() => {
        if (selectedCountry) {
            ...
            if (country && country.currency && country.symbol && country.codes) {
                ...
                calculatePricing(country.symbol, isDeveloping, country.codes)
                    .then((newPricing) => {
    ...
    const handleSearchSubmit = (e) => {
        ...
        const matchingCountry = countries.find(
            (c) => c.country.toLowerCase() === searchTerm.toLowerCase() || c.currency === searchTerm.toUpperCase()
        );
        if (matchingCountry) {
            setSelectedCountry(matchingCountry.country);
            setSearchTerm('');
            ...
            <div className="cont w-full">
                <div className="w-full cont lg:flex-row items-center gap-4 justify-between">
                    ...
                    <div className="flex items-end flex-col gap-2 w-full md:w-[350px]">
                        {selectedCountryData && selectedCountryData.isdeveloping === true && (
                            <p className="text-lg w-full">50% off for developing nations</p>
                        )}
                        <form onSubmit={handleSearchSubmit} className="relative w-full custom-dropdown">
                    ...
                    <div className="w-full flex flex-col lg:flex-row gap-8">
                                ...
                                {pricing.isDeveloping ? (
                                    <div className="flex flex-col">
                                        ...
                                        <p className="text-lg text-gray-500 line-through">
                                            {activeTab === 'monthly'
                                                ? `${pricing.originalMonthly}/month`
                                                : `${pricing.originalYearlyMonthly}/month`}
                                        </p>
                                    </div>
                                ) : (
                                    <div>
                                        ...
                                        {activeTab === 'yearly' && (
                                            <p className="text-lg text-gray-500 line-through">{`${pricing.originalMonthly}/month`}</p>
                                        )}
                                    </div>
                                )}
                            </div>
                                                ...
                                                <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 hidden group-hover:block z-50 w-[220px]">
                                                    <div className="relative">
                                                        <div className="rounded bg-black text-white text-sm px-3 py-2 shadow">
                                                            A single action your workflow completes, like sending an
                                                            email or updating a sheet.
                                                        </div>
                                        ...
                                        <li className="flex items-center gap-2">
                                            ...
                                            <div className="relative group inline-flex items-center">
                                                ...
                                                <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 hidden group-hover:block z-50 w-[220px]">
                                                    <div className="relative">
                                                        <div className="rounded bg-black text-white text-sm px-3 py-2 shadow">
                                                            Credits are used to run paid built-in plugs like SMS,
                                                            WhatsApp, or AI agents, and can also extend your workflow by
                                                            converting to tasks when needed.
                                                        </div>
                                                        <div className="w-2 h-2 bg-black rotate-45 absolute -bottom-1 left-1/2 -translate-x-1/2"></div>
                                                    </div>
                                                </div>
                        ...
                        <div className="lg:w-[40%] cont gap-8 border custom-border p-6 lg:p-12 bg-white">
                            ...
                            <Link
                                ...
                                className="w-[50%]"
                            >
                                <button className="w-full btn btn-accent">Contact Sales</button>
                            </Link>
