...
export default function AiWorkflowDemo({ compact = false }) {
    ...
    // Start animation when the demo window enters viewport
    useEffect(() => {
    ...
    return (
        <div
            ref={windowRef}
            className="bg-white rounded-[18px] border border-black/[0.08] shadow-[0_2px_4px_rgba(0,0,0,0.04),0_10px_36px_rgba(0,0,0,0.08),0_28px_72px_rgba(0,0,0,0.05)] overflow-hidden text-left flex flex-col"
        >
            {/* Chrome */}
            ...
            <div className="relative flex-1 flex flex-col overflow-hidden">
                ...
                <div
                    ref={chatRef}
                    className={`${
                        compact ? 'h-[300px]' : 'h-[390px] max-md:h-[260px]'
                    } overflow-y-auto p-[22px_22px_6px] flex flex-col gap-3.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden`}
                    aria-live="polite"
                >
                    {/* User msg */}
                    <div
                    ...
                    >
                        ...
                        <div className="relative bg-[#111] text-white text-[13px] leading-[1.56] px-[15px] py-[11px] rounded-[14px_4px_14px_14px] max-w-[74%]">
                            ...
                            {typed.length < USER_PROMPT.length && (
                                <span className="inline-block w-[1.5px] h-[1em] bg-white/80 ml-[1px] align-text-bottom animate-pulse" />
                            )}
                        </div>
                    </div>
                ...
                {/* Input area */}
                <InputArea chips={CHIPS} compact={compact} />
            </div>
        </div>
    );
}
