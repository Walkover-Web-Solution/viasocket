...
export default function HeadComp({ canonicalUrl }) {
    ...
    return (
        <>
            ...
            />
            <Script
                id="google-ads-signup-conversion"
                strategy="afterInteractive"
                dangerouslySetInnerHTML={{
                    __html: `function gtag_report_conversion(url) { var callback = function () { if (typeof(url) != 'undefined') { window.location = url; } }; gtag('event', 'conversion', { 'send_to': 'AW-16852796533/PQ6RCKGzwdQaEPWIheQ-', 'event_callback': callback }); return false; }`,
                }}
            />
        </>
    );
